...
class PChart
{
	...
	public $Errors         = NULL;
	public $ErrorFontName  = 'pf_arma_five.ttf';
		...
		elseif ( $Mode == 'GD' )
		 {
		  ...
		  foreach($this->Errors as $key => $Value)
		   {
		    $Position  = imageftbbox($this->ErrorFontSize,0,Kohana::config('pchart.fonts_folder').$this->ErrorFontName,$Value);
		    $TextWidth = $Position[2]-$Position[0];
		    if ( $TextWidth > $MaxWidth ) { $MaxWidth = $TextWidth; }
		   }
		  ...
		  foreach($this->Errors as $key => $Value)
		   {
		    imagettftext($this->Picture,$this->ErrorFontSize,0,$this->XSize-($MaxWidth+15),$YPos,$C_TextColor,Kohana::config('pchart.fonts_folder').$this->ErrorFontName,$Value);
